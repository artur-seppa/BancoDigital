...
public class TransferirCorrenteView implements ActionListener{
	...
	private static TransferirCorrenteView objCadastro = new TransferirCorrenteView();
	...
	public void imprimirTelaTransferirCorrente(DadosController dados, int id,
	...
	private void placeComponents(JPanel panelTransferir) {
		tituloTransferir.setText("Transferir Conta Corrente");
		...
		if(src == TransferirButton) {
			...
			if( saque >= 0.00d && !NameText.getText().isEmpty() ){
				if(correnteController.saldoContaCorrente(IDUsuario) >= saque) {
					sucesso = correnteController.transferirContaCorrente(IDUsuario,
